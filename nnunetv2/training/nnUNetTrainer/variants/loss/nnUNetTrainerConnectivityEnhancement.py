...
from nnunetv2.training.loss.compound_losses import DC_ConnectivityEnhancement_and_CE_loss
...
class nnUNetTrainerConnectivityEnhancement(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.weight_connectivityenhancement = 0.1 # This is the default value, you can change it if you want
        if self.label_manager.has_regions:
            raise NotImplementedError("trainer not implemented for regions")

    def _build_loss(self):
        if self.label_manager.ignore_label is not None:
            warnings.warn('Not support for ignore label')
        loss = DC_ConnectivityEnhancement_and_CE_loss(soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice,
                                                        'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                                                      soft_connectivity_enhancement_kwargs={'batch_dice': self.configuration_manager.batch_dice,
                                                           'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                                      ce_kwargs={}, weight_ce=1, weight_dice=1, weight_connectivity_enhancement=self.weight_connectivityenhancement,
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l, cel = self.loss(output, target, skel)
        ...
        if self.grad_scaler is not None:
            ...
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
            self.optimizer.step()
        return {'loss': l.detach().cpu().numpy(), 'cel': cel.detach().cpu().numpy()}
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        return {'loss': l.detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard, 'cel': cel.detach().cpu().numpy()}